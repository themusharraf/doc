...
import { Author, getAllBlockchainStaticPaths, getBlockchainForSlug } from "@/lib/markdown";
...
export async function generateMetadata(props: PageProps) {
  ...
  const res = await getBlockchainForSlug(slug);
  if (!res) return null;
...
export async function generateStaticParams() {
  const val = await getAllBlockchainStaticPaths();
  if (!val) return [];
  return val.map((it) => ({ slug: it }));
}

export default async function BlockchainPage(props: PageProps) {
  const params = await props.params;
  ...
  return (
    <div className="lg:w-[60%] sm:[95%] md:[75%] mx-auto">
      <Link
        ...
        href="/blockchain"
      >
        <ArrowLeftIcon className="w-4 h-4 mr-1.5" /> Back to blockchain
      </Link>
